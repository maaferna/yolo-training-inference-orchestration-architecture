...
Deep learning workloads involving PyTorch, CUDA, NVIDIA drivers, Ultralytics YOLO, and multi-GPU training are highly sensitive to operating system compatibility, driver versions, CUDA runtime configuration, and multiprocessing behavior.

During development, alternative environments were considered or tested. Windows was suitable for simpler single-device execution, but multi-GPU and DDP-oriented workflows introduced additional operational complexity. Other Linux distributions can work, but may require more manual dependency resolution depending on driver, CUDA, PyTorch, and package compatibility.

Ubuntu provides a more predictable baseline for NVIDIA driver installation, CUDA toolkit compatibility, PyTorch GPU execution, Docker-based GPU workloads, multi-GPU training experiments, Linux-native filesystem and process behavior, and reproducible deployment on GPU workstations or servers. This does not make Ubuntu the only valid option; it makes it the baseline that reduces runtime friction for this class of workload. The full reasoning is in [`02-system-architecture.md`](./docs/architecture/02-system-architecture.md).
...
| Annotation & Datasets | CVAT, Roboflow | Dataset preparation feeding the configuration layer |
...
The same map, with one line of purpose per document and the evolution section beside it, is kept
...
Every path ends in the evolution section, because the outcome of the limitations is the part
worth arriving at.
...
5. `docs/evolution/00-what-came-next.md`, then `docs/evolution/07-roadmap-realized.md`
...
6. `docs/evolution/01-submit-poll-execution.md`, then `docs/evolution/03-service-contracts.md`
...
6. `docs/evolution/02-model-registry-and-promotion.md`, then `docs/evolution/04-detection-metrology.md`
...
5. `docs/evolution/07-roadmap-realized.md`, then the ADRs marked `revision`
