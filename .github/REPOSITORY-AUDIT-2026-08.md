...
| M11 | ~~**The `ADR-001` collision outlived M2.**~~ **Resolved.** M2 fixed `docs/architecture/` but the second `ADR-001` stayed, unindexed, and listed two "future" ADRs whose numbers already belonged to the shared-storage and FastAPI decisions. It is now `ADR-008-path-translation-layer.md`, indexed in the ADR README, with its related-ADR list pointing at the real ADR-002 and ADR-003. The gate's duplicate-prefix check only covers `docs/architecture/`, not `adr/`. Found while aligning `CLAUDE.md` with disk; `docs/README.md` also still said "20 documents" after M10 and now says 21. | `adr/ADR-008-path-translation-layer.md` · `adr/README.md` · `docs/README.md:9,12` |
...
6. ~~**M5, M6**~~ — **done.** The eight broken links are repaired.
...
All 2 critical, 7 high, 11 medium and 7 low findings are closed. `./scripts/validate-sanitization.sh`
