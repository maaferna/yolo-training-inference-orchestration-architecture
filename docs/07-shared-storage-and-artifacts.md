...
### 8. Generated YAML Configurations (Django)

**Location**: `/shared_storage/configs/`

**Files**:
```
yaml_1717857600.yaml  # Generated by Django DatasetConfig model
yaml_1717857700.yaml  # Associated with specific ProjectConfiguration
yaml_1717857800.yaml  # Ultralytics-compatible YOLO config
```

**Purpose**: Centralized configuration management through Django ORM models.
...
**Size**: Typically 0.5-2 KB each (YAML is compact)

**Lifecycle**:
- Created: By Django DatasetConfig model when user configures project
- Accessed: By FastAPI training service (reads YAML path from training request)
- Retained: Indefinitely (small size, valuable for reproducibility)
- Referenced: By training requests and best_model_ref.json

**Format Note**: `names` field must use inline-style lists (not block-style) for Ultralytics compatibility.
