...
## Inference Result Synchronization & Path Translation Layer

**Responsibility**: Designed the critical integration layer ensuring that inference results generated by FastAPI (in its container filesystem) are properly copied, organized, and made accessible to Django (in a different container) through shared Docker volumes for visualization and download by end users.

### The Multi-Layer Path Coordination Challenge

**Problem**: 
- FastAPI generates inference results in `/app/compute_service/outputs/` (container coordinate system)
- Django needs to access results from `/app/web_service/outputs/` (Django container coordinate system)
- Users must access via `/media/deep_learning_outputs/outputs/` (public URL coordinate system)
- Host filesystem uses yet another path: `/home/user/outputs/`

All four paths reference the **same files**, but Docker mounts create a coordinate system translation challenge.
...
```python
# Map between 4 coordinate systems:
FastAPI container:  /app/compute_service/outputs/run_001/
            ↓
Host filesystem:    /home/user/outputs/run_001/
            ↓
Django volume:      /app/web_service/outputs/run_001/
            ↓
Public URL:         /media/deep_learning_outputs/outputs/run_001/
```

- Implemented PathTranslator class that centralizes all mapping logic
- Prevents hardcoded paths scattered across codebase
- Validates coordinates at each layer to catch misconfigurations

**Race Condition Prevention**:

- Discovered that Django would attempt to copy files while FastAPI was still writing
- Implemented `wait_for_path()` polling function that:
  - Checks directory exists
  - Verifies required files present (metrics.json, detections.csv)
  - Waits for file count to stabilize (unchanged for 3 consecutive checks)
  - Returns only when FastAPI has finished writing

**Metadata Persistence Strategy**:

- Store ONLY final-layer paths in database (volume paths and public URLs)
- Never store intermediate container or host paths
- Validates paths during ORM save() to enforce constraint
- Reduces confusion and prevents serving stale/incorrect URLs

### Error Handling Strategy

**Model Discovery Failures (HTTP 404)**:
```
Problem: get_best_model() returns None
Solution: Return detailed response including:
  - What we searched for (project, yolo_version, yolo_size)
  - Available models matching this project
  - Suggestion: "Train a model first or check version name"
```

**Parameter Validation Failures (HTTP 422)**:
```
Problem: Django sends "yolo_version_size" but FastAPI expects "model_version"
Solution: Align parameter names, add validation errors to response
```

**File Synchronization Timeouts**:
```
Problem: FastAPI still writing while Django tries to copy
Solution: Configurable timeout (default 5 minutes) with meaningful error message
```

### Lessons Learned & Portfolio Language

> "Designed the inference result synchronization layer coordinating Docker path mappings across 4 coordinate systems (host filesystem, 2 container mounts, and public URLs). Discovered race condition where Django would copy files while FastAPI was still writing; implemented polling-based stability detection ensuring write completion before copying. Developed PathTranslator abstraction centralizing path logic and preventing configuration errors. This demonstrates deep understanding of Docker architecture, distributed system coordination, and error handling in cross-service integration."

### Interview Talking Points

**Technical Depth**:
- "Docker containers have isolated filesystems. When two containers share files via bind mounts and volumes, you get multiple coordinate systems for the same underlying files. I had to design a layer that translates between them..."

**Problem Discovery**:
- "We noticed occasional corrupted or incomplete inference results. The root cause was subtle: Django didn't wait for FastAPI to finish writing before copying. I implemented a stability detector that polls the filesystem and only signals completion once file count stabilizes..."

**Design Thinking**:
- "Rather than scatter path translations throughout the code, I created a PathTranslator class that handles all 4-layer mappings. This prevents configuration errors and makes the system easier to maintain..."

**Risk Management**:
- "I identified that metadata stored in the database could reference phantom paths if we mixed coordinate systems. I added validation logic to ensure we only persist final-layer paths—the ones that are actually accessible..."
...
> "I designed a GPU-accelerated model training and inference orchestration system with microservice architecture. The system separates Django web server from FastAPI compute service, enabling independent scaling. I implemented multi-seed training strategy for statistical rigor and designed a continuous improvement pipeline with baseline comparison. A critical subsystem I built was the inference result synchronization layer—it coordinates Docker path mappings across 4 coordinate systems (host, FastAPI container, Django volume, public URL), implementing race-condition-safe file copying with polling-based stability detection. A key achievement was identifying a subtle race condition in concurrent model updates and proposing atomic operation solutions. I documented a multi-phase evolution strategy from synchronous single-GPU MVP to distributed Kubernetes infrastructure with enterprise-scale observability."
